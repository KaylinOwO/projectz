...
size_t* CBaseEntity::GetMyWeapons()
{
	DYNVAR(pHandle, DWORD, "DT_BaseCombatCharacter", "m_hMyWeapons");
	return (size_t *)gInts.EntList->GetClientEntityFromHandle(pHandle.GetValue(this));
}
...
{
	DYNVAR_RETURN(Vector, this, "DT_BasePlayer", "localdata", "m_vecPunchAngle");
}
...
Vector CBaseEntity::GetHitboxPosition(int iHitbox)
{
	...
	if (!this->SetupBones(matrix, 128, 0x100, gInts.globals->curtime))
		return Vector();
